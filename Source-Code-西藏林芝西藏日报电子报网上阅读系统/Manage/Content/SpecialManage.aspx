<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_SpecialManage, App_Web_mrqqbq1i" masterpagefile="~/Manage/I/Default.master" debug="true" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <ZL:Repeater ID="RPT" runat="server">
            <ItemTemplate>
              <tr data-layer="1" data-id='<%#Eval("SpecID") %>' onclick='LoadChilds(this,<%#Eval("SpecID") %>)'>
                  <td><%#Eval("SpecID") %></td>
                  <td><a href='SpecContent.aspx?SpecID=<%#Eval("SpecID") %>'><span class='<%#GetIcon() %>'></span></a> <a title="前台预览" href='/Special_<%# Eval("SpecID")%>/Default.aspx' target="_blank"><%#Eval("SpecName") %></a></td>
                  <td><%#Eval("SpecDir") %></td>
                  <td><%#Eval("SpecDesc") %></td>
                  <td>
                      <a href='AddSpec.aspx?Action=AddCh&ID=<%# Eval("SpecID")%>'>添加子专题</a>
                        |
                      <a href='AddSpec.aspx?Action=Modify&ID=<%# Eval("SpecID")%>'>修改</a>
                      |
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script>
        ...
        var Temp = "<tr data-layer='@layer' data-pid='@pid' data-id='@id' onclick='LoadChilds(this,@id)'><td>@id</td><td>@layerhtml<img src='/Images/TreeLineImages/t.gif'><a href='SpecContent.aspx?SpecID=@id'><span class='@imgicon'></span></a> <a title='前台预览' href='/Special_@id/Default.aspx' target='_blank'>@SpecName</a></td><td>@SpecDir</td><td>@SpecDesc</td><td><a href='AddSpec.aspx?Action=AddCh&ID=@id'>添加子专题</a> | <a href='AddSpec.aspx?Action=Modify&ID=@id'>修改</a> | <a href='SpecContent.aspx?SpecID=@id'>专题内容</a> | <a title='前台预览' href='/Special_@id/Default.aspx' target='_blank'>浏览</a> | <a href='javascript:;' onclick='DelCofim(@id)'>删除</a></td></tr>";
