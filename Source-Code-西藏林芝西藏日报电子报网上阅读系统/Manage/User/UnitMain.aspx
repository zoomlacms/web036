<%@ page language="C#" autoeventwireup="true" inherits="Manage_User_UnitMain, App_Web_sqzbmv5z" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div class="margin_b2px table table-bordered">
        <asp:CheckBox runat="server" ID="OnlyUnit_Chk" Checked="false" Text="仅显示已分成用户" OnCheckedChanged="OnlyUnit_Chk_CheckedChanged" AutoPostBack="true" />
    </div>
      <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10"  EnableTheming="False" 
           ...
           <asp:TemplateField HeaderText="用户名">
            ...
            <%--    <asp:BoundField HeaderText="消费金额" DataField="AMount" />--%>
                <asp:TemplateField HeaderText="下级消费金额"><ItemTemplate><%#Eval("ChildAMount","{0:f2}")+"<span class='remind_g_x'>"+Eval("Remind")+"</span>" %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="状态"><ItemTemplate><%#GetState() %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="无提成"><ItemTemplate><%#Eval("RealUnit0","{0:f2}") %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="可提成10%">
                    <ItemTemplate><%#Eval("RealUnit10","{0:f2}")+GetUnit(Eval("RealUnit10","{0:f2}"),0.1) %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="可提成20%"><ItemTemplate><%#Eval("RealUnit20","{0:f2}")+GetUnit(Eval("RealUnit20"),0.2) %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="可提成30%"><ItemTemplate><%#Eval("RealUnit30","{0:f2}")+GetUnit(Eval("RealUnit30"),0.3) %></ItemTemplate></asp:TemplateField>
                <asp:TemplateField HeaderText="总计提成金额"><ItemTemplate><span class="unit"><%#Eval("RealUnit","{0:f2}") %></span></ItemTemplate></asp:TemplateField>
           ...
           <div class="rd_green">无提成：未达到最低分成要求或已被下级提成掉30%</div>
