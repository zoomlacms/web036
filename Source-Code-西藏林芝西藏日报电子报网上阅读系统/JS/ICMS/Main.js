...
function ShowMain(leftUrl, rightUrl, obj) {//优先加载右边iframe
    if (rightUrl && rightUrl != "") {
        ...
        temp.src = "/Manage/wait.html?ver=3";
    ...
    setTimeout(function () {
        if (leftUrl && leftUrl != "") {
            ...
            lastLeft = leftUrl;
            //关键词记忆
            setTimeout(function () {myFrame.SetKeyword(currentFrameTabId); $(".ascx_key").bind("blur paste clip", function () { myFrame.SaveKeyword(currentFrameTabId, $(this).val()); }); }, 200);
        }
    }, 100);
